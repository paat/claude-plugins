...
| `TRIBUNAL_CODEX_EFFORT` | `medium` | Reasoning effort passed explicitly through `model_reasoning_effort`. An environment value overrides this default. The normal Codex leg also forces isolated user configuration and a read-only sandbox. |
| `TRIBUNAL_CODEX_SANDBOX_BYPASS` | `off` | Set to `on` to pass `--dangerously-bypass-approvals-and-sandbox` to `codex exec`. Needed in containers where unprivileged user namespaces are disabled (bwrap cannot create codex's sandbox) — there sandboxed codex cannot run any command and emits a vacuous 0-findings/`BLOCK`-or-`NEEDS_WORK` verdict (issues #167/#168/#171). Any leg (not just Codex) whose review has zero findings but a blocking verdict is now downgraded to a leg **error** instead of reaching arbitration, so a repo-blind reviewer can no longer silently drop out of the panel; enabling this knob restores a real repo-walking review. |
