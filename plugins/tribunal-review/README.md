...
Multi-provider code review plugin for Claude Code and Codex. **By default** it runs Codex (GPT-6 Astra at medium effort), **Grok** (grok-4.5) via the xAI Grok CLI, and **Claude** (sonnet) via the host Claude Code CLI, then the calling context arbitrates the findings and issues the verdict. **Gemini** (3 Pro Preview, web/CVE search), **DeepSeek-V4-Pro**, the **OpenCode Go GLM-5.1** leg, and **Qwen** (qwen3.7-plus) via the Qwen Code CLI are available **opt-in**. To enable DeepSeek, set `TRIBUNAL_DEEPSEEK=on`; it defaults to `deepseek/deepseek-v4-pro` on the direct API's independent transport (issue [#461](https://github.com/paat/claude-plugins/issues/461)); GLM remains off. Codex and Grok **walk the repo**; Codex uses isolated user configuration and unrestricted execution inside the development-container security boundary while its review prompt prohibits file changes. Grok walks under a tools allowlist with host config isolated (web search off); sandbox defaults to `none` because the development container is the security boundary (kernel `read-only` needs nested user namespaces and fails inside many containers — issue #378; opt in with `TRIBUNAL_GROK_SANDBOX=read-only`). **Claude** is the panel's **diff-only** lens (run from a scratch dir with all tools disabled), restoring the harness/context diversity the walking legs give up. The opt-in GLM/Gemini legs are also diff-only; the opt-in DeepSeek and Qwen legs walk the repo on their own configured transports.
...
- [Grok CLI](https://github.com/xai-org/grok-cli) (`grok`, "Grok Build") — needed for the **Grok leg, which runs by default** (disable with `TRIBUNAL_GROK=off`). Repo-walking under a tools allowlist (`read_file,list_dir,grep`) with `--sandbox none` by default (container is the security boundary; set `TRIBUNAL_GROK_SANDBOX=read-only` where unprivileged user namespaces work) and `--permission-mode bypassPermissions` (issue #378; `dontAsk` cancels multi-turn inspect). Host Claude/Grok user config is isolated to a scratch HOME while `auth.json` is **copied** (not symlinked) and any OIDC refresh is written back under `flock` so Max/SuperGrok autonomous loops keep a live session (issue #374). Web search off. The runner pins a session id, bounds inspect turns, and on progress-only/timeout **resumes tools-off** for a schema verdict (never reports success on a plan announcement alone; issue #331). Non-`EndTurn` stop reasons without a schema verdict fail as a dead leg (not a quiet empty review). Auth via the CLI's own login (`grok login`, or `grok login --device-code` headless); preflight skips a present CLI with no usable session. Max subscription users should prefer a durable OIDC session (`refresh_token` in `~/.grok/auth.json`) over pasting `XAI_API_KEY` (console billing rail). If the CLI is missing or unsigned-in the leg self-emits an error JSON and the run degrades to the remaining quorum.
...
them before launching the host). The default panel is **Codex + Grok + Claude**; Gemini, DeepSeek, GLM,
and Qwen stay **off** until you opt in.
The `opencode-go` DeepSeek gate exists because of China hosting; the direct `deepseek/*` provider is also PRC-operated. DeepSeek runs `--agent plan` with `cwd = $REPO_ROOT` and can read the whole repository, not just the attached diff. For projects with data-residency constraints, enabling this leg is the operator's decision.
...
| `TRIBUNAL_GROK` | `on` | Set to `off` to skip the Grok leg (xAI Grok CLI; repo-walking with tools allowlist; sandbox default `none` + `bypassPermissions` — issue #378; host config isolated with durable OIDC auth copy/write-back — issue #374; inspect→tools-off finalize resume for deterministic completion — issue #331). **On by default**; when off the arbiter reports Grok as `disabled`, not failed. Only the literal `off` disables. Autonomous Max/OAuth users need a live `grok login` session (refresh token), not re-login every round. |
...
| `TRIBUNAL_GROK_SANDBOX` | `none` | Grok CLI `--sandbox` profile. Defaults to `none` (development container is the boundary). Set `read-only` on hosts with working unprivileged user namespaces. Also honors `GROK_SANDBOX` when this variable is unset. |
| `TRIBUNAL_GROK_PERMISSION_MODE` | `bypassPermissions` | Grok CLI `--permission-mode`. `dontAsk` cancels multi-turn inspect in practice (issue #378). |
...
| `TRIBUNAL_MIN_OK_LEGS` | `1` | Per-environment APPROVE floor: at least this many provider legs must seal as `ok` before `APPROVE` is valid (integer 1..7; unset/empty ⇒ 1). Set on the container/host only — never read from the repository under review. A documented lower value is the environment's degraded quorum. Membership still uses the `TRIBUNAL_<PROVIDER>` toggles. |
...
export TRIBUNAL_DEEPSEEK=on  # add the opt-in DeepSeek leg on the direct API
