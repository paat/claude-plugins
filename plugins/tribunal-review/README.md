...
- [Grok CLI](https://github.com/xai-org/grok-cli) (`grok`, "Grok Build") — **optional**, only for the **Grok leg, which is opt-in** (`TRIBUNAL_GROK=on`; off by default). Repo-walking but read-only (`--disallowed-tools write,edit`) with web search off. Auth via the CLI's own login (`grok login`). If the CLI is missing the leg self-emits an error JSON.
...
| `TRIBUNAL_GROK` | `off` | Set to `on` to enable the Grok leg (xAI Grok CLI, repo-walking read-only). **Off by default**; when off the arbiter reports Grok as `disabled`, not failed. Only the literal `on` enables. |
...
export TRIBUNAL_GROK=on                              # add the opt-in Grok leg (xAI Grok CLI, read-only)
