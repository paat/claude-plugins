...
| `TRIBUNAL_DEEPSEEK_MODEL` | `opencode-go/deepseek-v4-pro` | Model passed to `opencode run -m`. The supported reopen path is `TRIBUNAL_DEEPSEEK=on TRIBUNAL_DEEPSEEK_MODEL=deepseek/deepseek-v4-pro`, which uses the direct DeepSeek API and independent transport (authenticate with `DEEPSEEK_API_KEY` or `opencode auth login` → DeepSeek). |
| `TRIBUNAL_GLM` | `off` | Set to `on` to enable the OpenCode Go GLM leg. **Off by default**; it remains opt-in and is not a DeepSeek replacement. Only the literal `on` enables. |
