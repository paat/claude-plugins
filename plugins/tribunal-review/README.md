...
| `TRIBUNAL_CLAUDE_MODEL` | `sonnet` | Model passed to `claude -p --model` for the diff-only leg. Accepts an alias (`sonnet`, `haiku`, `opus`) or a full id (e.g. `claude-sonnet-5`). Defaults to `sonnet` — fast/cheap and decorrelated from the Opus arbiter; setting it to `opus` maximizes reviewer↔arbiter correlation. |
