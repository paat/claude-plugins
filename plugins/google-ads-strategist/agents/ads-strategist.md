...
If you hit a problem with the **plugin itself** (not the ad work), first run `gh issue list --repo paat/claude-plugins --search "in:title google-ads-strategist:"`, then file with `gh issue create --repo paat/claude-plugins --title "google-ads-strategist: <short title>" --body-file <path>`. GitHub issues replaced the local `PLUGIN_ISSUES.md` workflow — the per-project file was never aggregated across downstream projects.
