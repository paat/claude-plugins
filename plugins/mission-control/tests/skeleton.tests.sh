...
t "plugin.json parses"          jq -e '.name == "mission-control" and .version == "0.3.0"' "$PLUGIN/.claude-plugin/plugin.json"
t "marketplace entry exists"    jq -e '.plugins[] | select(.name == "mission-control") | .version == "0.3.0"' "$REPO_ROOT/.claude-plugin/marketplace.json"
