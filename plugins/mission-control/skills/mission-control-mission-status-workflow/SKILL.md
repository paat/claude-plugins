---
name: mission-control-mission-status-workflow
description: "Run /mission-status workflow from mission-control; alias /mission-control:mission-status."
---

# /mission-control:mission-status Codex Workflow

This generated skill is the Codex-native plugin surface for `/mission-control:mission-status`.
Also use it when the user invokes `/mission-status` or asks for the same workflow by name.

Source command: `../../commands/mission-status.md`
...
- Command aliases: `/mission-control:mission-status`, `/mission-status`
- Source description: Read-only view of mission-control state - slot occupancy, pool quotas and backoffs, cooldowns, pending admissions with veto deadlines, and recent dispatch outcomes. Usage: /mission-status [config-path]
