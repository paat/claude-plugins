...
contains "$META_SKILL" 'the instant a background leg is dispatched' 'Meta skill writes the handoff at dispatch, not later'
contains "$META_SKILL" 'before this turn ends with that leg live' 'Meta skill commits the handoff before ending a turn with a live leg'
...
contains "$META_SKILL" "The brief's stop conditions, gate blockers, and genuine judgment calls still stop" 'Meta skill keeps brief stop conditions and blockers binding'
...
contains "$META_SKILL" 'Do not burn the session on preflight before the first dispatch' 'Meta skill forbids preflight burning the session before dispatch'
...
contains "$META_REFS/handoff-template.md" 'task/job id' 'In-flight legs carry the leg task or job id'
