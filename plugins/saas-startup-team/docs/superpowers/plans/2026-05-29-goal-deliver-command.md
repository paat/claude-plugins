# `/goal-deliver` Command Implementation Plan (lean playbook)

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development or superpowers:executing-plans. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Add a `/goal-deliver` playbook-macro command to the saas-startup-team plugin that expands a set of tasks into the deliver-to-production workflow (plan into chunks → per chunk `/improve` + closing tribunal loop + merge → monitor deploy), pairing with built-in `/goal` for autonomy.

**Architecture:** A single markdown command (`commands/goal-deliver.md`) — a prompt, not an engine. No helper scripts and no `plan.json` state machine (deliberately dropped so the orchestrator keeps judgment over chunking/ordering). Covered by cross-file consistency assertions in the existing `tests/run-tests.sh`.

**Tech Stack:** Claude Code slash command, `gh` CLI, the `/improve` command, and the `tribunal-review` plugin (hard dependency).

---

## Spec

`plugins/saas-startup-team/docs/superpowers/specs/2026-05-29-goal-deliver-command-design.md` (lean revision). Read it first.

## File Structure

| File | Responsibility | Action |
|---|---|---|
| `plugins/saas-startup-team/commands/goal-deliver.md` | The playbook-macro command | Create |
| `plugins/saas-startup-team/tests/run-tests.sh` | Add Suite **T** (`test_goal_deliver`), register in `main()`. A–S are taken; use T. | Modify |
...
| `plugins/saas-startup-team/README.md` | Document `/goal-deliver` | Modify |

Paths are relative to repo root `/mnt/data/ai/claude-plugins`.

---

## Task 1: `commands/goal-deliver.md` + Suite T (TDD via consistency assertions)

**Files:**
- Create: `plugins/saas-startup-team/commands/goal-deliver.md`
- Test: `plugins/saas-startup-team/tests/run-tests.sh`

- [ ] **Step 1: Write the failing consistency suite.** Add before the final `main "$@"` line (after the last suite function):
...
Register in `main()` after `test_migrate_handoff_names`:
...
- [ ] **Step 2: Run to verify it fails.** `bash plugins/saas-startup-team/tests/run-tests.sh 2>&1 | grep -A2 "Suite T"` → FAIL (`T1: goal-deliver.md exists`).

- [ ] **Step 3: Write the command file** exactly as in **Appendix A** below.

- [ ] **Step 4: Run to verify it passes.** `bash plugins/saas-startup-team/tests/run-tests.sh 2>&1 | grep -E "T[0-9]+|FAIL"` → all `T*` PASS, no `FAIL`.

- [ ] **Step 5: Commit.**
```bash
git add plugins/saas-startup-team/commands/goal-deliver.md plugins/saas-startup-team/tests/run-tests.sh
git commit -m "feat(saas-startup-team): /goal-deliver playbook-macro command"
```

---

## Task 2: Version bump + README + full suite

- [ ] **Step 1:** Bump `version` 0.36.0 → 0.37.0 in `plugins/saas-startup-team/.claude-plugin/plugin.json` AND the `saas-startup-team` entry of `.claude-plugin/marketplace.json`. Verify equal.
- [ ] **Step 2:** Add a `/goal-deliver` bullet to the README command list.
- [ ] **Step 3:** Run `bash plugins/saas-startup-team/tests/run-tests.sh` → `All tests passed!` (exit 0); `E3` semver passes for 0.37.0.
- [ ] **Step 4: Commit.**
```bash
...
git commit -m "chore(saas-startup-team): v0.37.0 — /goal-deliver command"
```

---

## Appendix A — full content of `commands/goal-deliver.md`
