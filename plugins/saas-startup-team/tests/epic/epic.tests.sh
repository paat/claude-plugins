...
test_epic() {
  ...
  assert_file_contains "EP15: post-merge deploy poll" "$skill" '--deploy-sha'
  assert_file_contains "EP15b: never latest run" "$skill" 'latest run'
  assert_file_contains "EP15c: live verify after deploy" "$skill" 'Live verify after deploy green'
  assert_file_contains "EP15d: close only after deploy+live" "$skill" 'Only after'
  assert_file_contains "EP15e: complete needs deploy+live" "$skill" 'deploy green on `merge_sha`'
  ...
  assert_file_contains "EP20c: invariants post-merge deploy" "$inv" 'deploy-sha'
  assert_file_contains "EP20d: invariants live-verify" "$inv" 'live-verify'
