...
assert_file_contains "LH5: pending flags remain durable" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" 'The flags remain durable in `.startup/law-registry.json`'
assert_file_contains "LH6: issue creation remains explicit" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" "Issue creation still requires an explicit subcommand"
assert_file_contains "LH7: flagged topic dependency requires primary evidence" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" "re-verify it from Tier A before using it"
...
assert_file_contains "LH10: context gathering is topic-targeted" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" "Do not inventory or load the newest files across every docs area"
...
assert_file_contains "LH17: command names deterministic structural validation" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" 'legal-verdict-gate.sh'
assert_file_contains "LH18: command enables validation mode" "$PLUGIN_ROOT/skills/lawyer/SKILL.md" '--validate'
