...
11. Apply edits:
    - For each target file, append the planned entries as dash-bullets in house style (`templates/learnings-style.md`), routing by rule severity: catastrophic rules go under `## Critical Landmines`; routine rules go under the best-fit failure-mode `##` section. Section routing is strictly append-only: if the target section already exists and is the last `##` block in the file, append there; if the target section exists but is not last, append to the end of that section without reordering anything above or below it; if the target section does not exist, create it as a new `##` heading at the very end of the file and append there. Never insert content mid-file or reorder existing entries. Create the file with a `# <heading>` H1 if it was a new topic.
