...
5. **First-time bootstrap** — only if the topic catalog is empty after step 4:
   ...
   - After confirmations, create each confirmed topic file with a `# <display heading>` H1 and a blank line underneath. Seed each new topic file with a `## Critical Landmines` section (placeholder comment `<!-- catastrophic rules only; stronger language allowed here -->`) followed by failure-mode `##` sections inferred from the cluster. Append every entry in the house style defined in `${CLAUDE_PLUGIN_ROOT}/templates/learnings-style.md` (`- <Label>: <rule> — <why>. Fix: <pattern>. (ref)`); reserve `ALWAYS`/`NEVER`/caps for `## Critical Landmines` only. During clustering, do not discard a line as obvious if it is project-specific, library/version-specific, exact-behavior, post-cutoff, counterintuitive, or provenance-tagged. Treat borderline entries as keepers: when unsure, keep and route rather than discard.
...
11. Apply edits:
    - For each target file, append the planned entries as dash-bullets in house style (`${CLAUDE_PLUGIN_ROOT}/templates/learnings-style.md`), routing by rule severity: catastrophic rules go under `## Critical Landmines`; routine rules go under the best-fit failure-mode `##` section. Section routing is strictly append-only and writes ONLY at the end of the file: route a catastrophic rule under `## Critical Landmines` and a routine rule under its best-fit failure-mode `##` section — but if that section is not already the LAST `##` block in the file, do NOT insert into it; instead append a fresh `## <section>` heading at the very end of the file and put the entry there (a section may therefore recur, which a later /learnings-compress run consolidates). Never insert content mid-file or reorder existing entries. Create the file with a `# <heading>` H1 if it was a new topic.
