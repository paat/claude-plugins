...
# /maintain — Autonomous Maintenance Playbook
...
**Delegation topology:** run `/goal-deliver` **inline** per issue so the supervisor
retains mutation and merge ownership. Current Claude Code agents may delegate bounded
browser-operator legs; Codex keeps those browser steps flattened.
...
PLUGIN_ROOT="${PLUGIN_ROOT:?set PLUGIN_ROOT to the installed saas-startup-team plugin}"
GIT_COMMON=$(cd "$(git rev-parse --git-common-dir)" && pwd)
mkdir -p "$GIT_COMMON/saas-startup-team"
...
## Model-Free Entry Gate

The thin command entrypoint has already run `scripts/workflow-probe.sh maintain`.
Reaching this playbook means the probe found new/changed triage input or a cached
final verdict whose supervisor finalization is incomplete under the current routing
schema. Its `cached_resumable` result is deliverable queue input and resumable work, not
a reason to stop. Do not repeat the predicate here.

Load `${CLAUDE_PLUGIN_ROOT}/references/workflows/routing-telemetry.md` now. The probe's
exit-3 path launches no worker and emits no worker event.

---

## Whole-Pass Lease

On a normal run, acquire a repository-wide pass lease **before** changing the persistent
worktree, `.git/info/exclude`, labels, state, or `active_role`. The lease lives in the
common Git directory so every checkout/worktree contends on the same identity:

```bash
REPO_ROOT=$(git rev-parse --show-toplevel)
GIT_COMMON=$(cd "$(git rev-parse --git-common-dir)" && pwd)
MAINTAIN_LEASE_DIR="$GIT_COMMON/saas-startup-team/leases"
MAINTAIN_OWNER_FILE="$MAINTAIN_LEASE_DIR/.owners/maintain-pass.owner"
bash "${CLAUDE_PLUGIN_ROOT}/scripts/single-flight.sh" \
  --acquire maintain-pass --state-dir "$MAINTAIN_LEASE_DIR" \
  --owner-file "$MAINTAIN_OWNER_FILE" --ttl-seconds 1800
```
...
`EXIT INT TERM HUP` trap that calls `single-flight.sh --release maintain-pass` with these
exact state/owner paths; still release explicitly on normal completion. Model tool calls
may run in separate shell PIDs, so the durable owner file—not a PID—is authoritative.

```bash
release_maintain_pass() {
  [ ! -s "$MAINTAIN_OWNER_FILE" ] || bash \
    "${CLAUDE_PLUGIN_ROOT}/scripts/single-flight.sh" \
    --release maintain-pass --state-dir "$MAINTAIN_LEASE_DIR" \
    --owner-file "$MAINTAIN_OWNER_FILE"
}
...
the examples above; `flock` suppresses duplicate launches, while this lease prevents
...
2. **Route, then triage read-only.** For each cache miss, write title/body/comments and
   labels to temporary local files and call `delivery-route.sh classify --mode
   autonomous`. A routine result may dispatch
   `subagent_type: "saas-startup-team:maintain-triage"` with profile `light`; record its
   Haiku/low start and terminal events without issue text. A deep classification or an
   `uncertain` cheap-triage result dispatches
   `subagent_type: "saas-startup-team:business-founder-maintain"` at Fable/high with
   profile `deep` for the final brief/verdict. It may write only a concise business
   brief/proposed workflow-spec delta; **you (the supervisor) apply all side-effects**.
   Claude may delegate judgment-free browser operations to the registered cheap browser
   agents while Fable retains the verdict; Codex keeps this flow flattened. Skip
   classification only when `triage-cache.jsonl` has a matching
   `{number, updatedAt,routing_schema_version}` final verdict.
   Under `--dry-run`, keep classification and both verdict phases in memory and suppress
   event, cache, brief, and artifact writes; `.startup/runs` must not be created.
...
7. **Write pass digest** to `.startup/maintain/runs/<run-id>.md`, heartbeat, then release
   the whole-pass lease with the same durable owner file. The release happens after the
   last mutation and before reporting/backoff. A terminal error path writes its outcome
   when possible and releases through the same cleanup path.
...
8. If `--once`, stop and report. Otherwise **back off** (default ~5 min) and repeat
   by reacquiring the whole-pass lease before the next pass touches the worktree, then
   continue from step 1. (A foreground interactive turn cannot sleep-and-resume across this
