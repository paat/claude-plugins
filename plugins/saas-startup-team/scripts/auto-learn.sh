...
# Base extraction instruction — delta-gated, house-style capture (see LEARN_MSG heredoc).
msg=$(cat <<'LEARN_MSG'
Read the file just written. Extract up to 3 reusable project learnings (tech stack decisions, coding conventions, error patterns, API gotchas, business/legal rules). Skip obvious knowledge (see the delta rule below). Find git root (git rev-parse --show-toplevel). Ensure CLAUDE.md exists with '# Project Learnings' H1 and '## Learnings' H2. List files in docs/learnings/*.md at git root (skip if dir missing); for each, read the first '#'/'##' heading line (fall back to filename stem with dashes→spaces if no heading) to build a topic catalog. For each candidate learning: (a) skip if semantically equivalent to any existing entry in any topic file or in '### Recent (unsorted)'; (b) if it clearly fits one existing topic file, append a dash-bullet to that file (place a catastrophic rule under that file's `## Critical Landmines` section if it has one; otherwise append at end of file); (c) otherwise ensure '### Recent (unsorted)' subsection exists under '## Learnings' with comment '<!-- Uncertain/new-topic learnings staged here. Run /saas-startup-team:learnings-migrate to organise into docs/learnings/*.md. -->' and append the dash-bullet there. Record only the DELTA — facts surprising to a competent model. Do NOT record general/textbook best-practice the model already applies; that only dilutes the real rules. If a candidate is a general standard or team convention worth enforcing (not project/library-specific), it does NOT belong in a topic file — instead append it to the '### Recent (unsorted)' staging area prefixed 'PROMOTE?: ' so it is never lost and a human can relocate it to the agent prompt Standards section. Never silently drop a tier-2 candidate. Calibration guard: KEEP anything project-specific, library/version-specific, exact-behavior (inheritance/typing facts), post-cutoff, counterintuitive, or tied to a #issue/incident/test — even if it feels obvious; when unsure, keep it. Write each kept entry in house style, one dash per line: - <Label>: <imperative rule> — <terse why>. Fix: <pattern>. Label is a canonical term or failure-mode handle (e.g. Idempotency, Token hygiene, Retry semantics). Keep the terse why; add Fix only when a concrete reusable action exists. Prefer canonical terms; spell out overloaded ones (atomic, fail-safe). Ration emphasis — use ALWAYS/NEVER/caps ONLY for a catastrophic landmine, never routine rules. Append at most a terse (#issue or file) ref, never a provenance sentence. ~25 words max excluding ref. If a candidate is a one-off permission/grant (time- or task-scoped access, not a durable rule), record it as '- Grant: <what> — scope: <scope>, expires: <YYYY-MM-DD>' so memory-gc auto-retires it once past. If a candidate is a human correction that contradicts a living doc (brief, launch-readiness, signoff), also append a one-line task to docs/human-tasks.md at git root naming the doc to reconcile. Max 3 new entries total. If nothing worth recording, do nothing.
LEARN_MSG
)
