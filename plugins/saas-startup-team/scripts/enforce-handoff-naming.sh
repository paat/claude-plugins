...
# Canonical format — include tech-plan (architect pass artifact, #381).
if [[ "$filename" =~ ^([0-9]{3}|[1-9][0-9]{3,})-(business-to-tech|tech-to-business|business-to-growth|growth-to-business|tech-plan)\.md$ ]]; then
  exit 0
fi
...
if [ -d "$handoff_dir" ]; then
  max=$(ls "$handoff_dir" 2>/dev/null \
    | grep -E '^([0-9]{3}|[1-9][0-9]{3,})-(business-to-tech|tech-to-business|business-to-growth|growth-to-business|tech-plan)\.md$' \
...
msg="Handoff filename '${filename}' is not valid. Handoffs must be named NNN-<direction>.md where NNN is a zero-padded number (3 digits up to 999, then 4+ digits) and <direction> is one of: business-to-tech, tech-to-business, business-to-growth, growth-to-business, tech-plan. Next available NNN: ${next_nnn}. Binaries (.pdf, .png) belong in .startup/attachments/; signoffs in .startup/signoffs/; reviews in .startup/reviews/."
