...
This generated skill is the Codex-native plugin surface for `/saas-startup-team:pause`.
Also use it when the user invokes `/pause` or asks for the same workflow by name.

Source command: `../../commands/pause.md`
...
- Command aliases: `/saas-startup-team:pause`, `/pause`
- Source description: Pause an active /startup loop so you can exit the session cleanly without completing the product. Resume with /startup. Usage: /pause [reason]
