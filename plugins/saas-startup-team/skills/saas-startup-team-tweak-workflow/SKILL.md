...
This generated skill is the Codex-native plugin surface for `/saas-startup-team:tweak`.
Also use it when the user invokes `/tweak` or asks for the same workflow by name.

Source command: `../../commands/tweak.md`
...
- Command aliases: `/saas-startup-team:tweak`, `/tweak`
- Source description: Direct-edit shortcut for trivial fixes on a completed product - typos, copy tweaks, small CSS nudges. Edits files directly (no agents). On main, creates a tweak/ branch and opens a PR; on a feature branch, commits to that branch and pushes (no new PR). For anything needing QA, use /improve instead. Usage: /tweak [description]
