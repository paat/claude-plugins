...
This generated skill is the Codex-native plugin surface for `/saas-startup-team:session-insights`.
Also use it when the user invokes `/session-insights` or asks for the same workflow by name.

Source command: `../../commands/session-insights.md`
...
- Command aliases: `/saas-startup-team:session-insights`, `/session-insights`
- Source description: Local-only scan of this project's Claude Code session logs for investor interventions (interrupts, /nudge, corrections) and agent friction (tool failures). Emits typed records + a Markdown report under .startup/insights/. No network, no issue filing, no arguments. Usage: /session-insights
