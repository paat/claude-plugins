...
This generated skill is the Codex-native plugin surface for `/saas-startup-team:monitor-nightly`.
Also use it when the user invokes `/monitor-nightly` or asks for the same workflow by name.

Source command: `../../commands/monitor-nightly.md`
...
- Command aliases: `/saas-startup-team:monitor-nightly`, `/monitor-nightly`
- Source description: Nightly automated monitor - sweeps failure markers + an optional project custom-checks script, files/dedups GitHub issues with reproduction context, persists state across runs. Usage: /monitor-nightly [--dry-run]
