...
This generated skill is the Codex-native plugin surface for `/saas-startup-team:learnings-migrate`.
Also use it when the user invokes `/learnings-migrate` or asks for the same workflow by name.

Source command: `../../commands/learnings-migrate.md`
...
- Command aliases: `/saas-startup-team:learnings-migrate`, `/learnings-migrate`
- Source description: Organise learnings staged in '### Recent (unsorted)' of CLAUDE.md into topic files under docs/learnings/. Bootstraps the topic catalog on first use. Non-destructive - flat pre-existing '## Learnings' bullets outside Recent are left alone.
