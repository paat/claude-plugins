...
Shared filing for investigate/replay/plugin defects is `scripts/issue-file.sh` (skill `issue-file`): open-issue dedup, optional `--pattern-key` with a `**Pattern:** \`key\`` body marker, optional `--source-repo` / `--source-escalate comment` after local success, PII park (exit 3), no post-create search gate. Nightly monitor still uses `scripts/monitor-dedup.sh`.
