...
This generated skill is the Codex-native plugin surface for `/payment-contract-tester:scaffold`.
Also use it when the user invokes that command or asks for the same workflow by name.

Source command: `../../commands/scaffold.md`
...
- Plugin: `payment-contract-tester`
- Command aliases: `/payment-contract-tester:scaffold`
- Source description: Detect a repo's payment stack + gateway, draft repo-adapted contract tests using the payment-contract-tester skill, self-verify them (green on current source, red on a broken copy), and wire CI + an optional pre-push hook. Never edits payment source.
